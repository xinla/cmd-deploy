...
function checkConfigScheme(configKey, configObj, privateKey) {
  ...
  if (neededKeys.length > 0) {
    errorLog(`配置文件中${configKey}环境缺少${neededKeys.join(',')}配置，请检查配置`)
    configValid = false
  }
  if (unConfigedKeys.length > 0) {
    errorLog(`配置文件中${configKey}环境的${unConfigedKeys.join(', ')}暂未配置，请设置该配置项`)
    configValid = false
  }
  return configValid
}
...
async function checkDeployConfig(deployConfigPath) {
  ...
  infoLog(`缺少部署相关的配置，请运行${underlineLog('deploy init')}创建部署配置`)
  return false
}

export { startLog, infoLog, successLog, errorLog, underlineLog, checkNodeVersion, checkDeployConfig }
